...
| 5 | BEQ-01：**未登记资产业务定位（行级隔离豁免）** | A) 全公司公共池，所有人可见（不补隔离）；B) 部门隔离（Selector 加 `get_queryset_for_user`，update 限创建人本部门或 dept_manager） | ✅ 已拍板（2026-09-17）：选 A——未登记资产为**全公司公共池**，列表/详情/更新对全部认证用户开放属预期行为，SC-4 对该模块豁免，越权修改风险一并接受，零代码改动；见 `审查报告_2026-09-13` BEQ-01 拍板记录 |
