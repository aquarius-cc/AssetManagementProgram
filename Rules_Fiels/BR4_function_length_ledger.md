...
| B2 | unregisteredasset/views.py | 272 | batch_delete | 79 | 推荐下沉 `UnregisteredAssetService.batch_delete(ids,op)`（与 batch_create 同模式,#17 分层收敛先例），View 仅校验 ids/上限+调 service+BatchResponseHelper(~10)；保守替代：View 内 `_delete_one_item`(~18)；docstring 压缩 | test_batch_delete_view.py（成功/四种失败结构/权限直接锚）+ test_b5_baseline_snapshot.py + test_concurrent.py | fail_items 结构（NOT_FOUND/STATUS_NOT_ALLOWED/VALIDATION_ERROR/INTERNAL_ERROR）；resolve_operator 上下文；下沉后与既有批量模式 DR-1 对齐 | 待拆分 |
