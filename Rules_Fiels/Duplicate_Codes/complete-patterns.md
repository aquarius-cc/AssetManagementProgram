...
### A-30. 【关闭 2026-09-22】包─模块同名影子死文件治理 + 护栏 G-5 通用化（B-11/B-14 应用级同型，§1.8 新发现义务登记）
- **编号冲突规避**：顺延登记（A-29 已占用），沿用 v2.9.23/v2.9.34 先例。
- **状态**：✅ 已关闭 | 关闭日期：2026-09-22 | 本批修复
- **判定**：B-11（assetmanagement 5 死文件）/B-14（usermanagement views.py）同型——「包优先解析使同名 .py 成死代码」在存量代码中仍有残留，且 B-11 收尾声明「根级仅剩正常模块（admin…）」不实（2026-09-22 扫描实证）。
- **新发现（实测，find_spec/is_package 证实）**：
  1. `apps/assetmanagement/admin.py`（201 行、28 处 `@admin.register`）与 `admin/` 包并存——包内 `asset_admin.py` 以 `@admin.register` 注册 16 模型（Storage/AssetType/Contract/Asset/OutAsset/RecycleAsset/DamagedAsset/WasteAsset/HardDiskSN + Broken/Lost/Found/Repair/AssetOperationLog/AssetStateLog），为 `admin.py` 9 模型注册的**超集**，`admin.py` 整文件死亡；Django `autodiscover` 导入解析到包，`admin.py` 从未被加载。
  2. `apps/authusermanagement/tests.py`（1 行占位 `# Create your tests here.`）与 `tests/` 包并存被遮蔽，包内 6 个测试文件全量生效。
  - 无影子风险例外（合法平模块，不拦截）：`authusermanagement/services.py`、`unregisteredasset/services.py`（无同名包）。
- **修复内容**：整文件删除上述两处 + 对应 `__pycache__` 残留 pyc（`admin.cpython-313.pyc`/若存在的 `tests` pyc）。删除前先跑 admin 注册冒烟基线（21 模型），删除后复跑**逐字节一致**——零注册变化。
- **护栏升级（G-5 通用检查，取代窄化方案）**：`scripts/check_duplicate_invariants.py` 新增 `check_g5_no_shadow_modules()`（约 10 行）——`rglob("__init__.py")` 递归枚举 `apps/` 全部包目录，断言同级 `<同名>.py` 不得存在；全面覆盖 B-11 的 5 个路径 + B-14 + 本次 admin/tests + 未来任何层级复现（含 `management/commands` 等深层嵌套）；取代 v2.9.24 A-23 附带建议的窄化 5 行 `services.py` 检查（v2.9.37 定案）。docstring 退出码同步为「G-1~G-3、G-5 任一命中阻断」。CI `duplicate-guard.yml:18` 直接跑本脚本，G-5 自动生效，零 CI 改动。
- **验证命令**：删除后 `python manage.py check` no issues（1 silenced）+ registry 21 模型与删除前逐字节一致；`pytest apps/authusermanagement apps/assetmanagement -q` 824 passed（基线无回归）；先红后绿——touch `usermanagement/services.py` → `[BLOCK] G-5` exit=1（1 项）→ 补 touch `assetmanagement/admin.py` → exit=1（2 项）→ 双删 → PASS exit=0；`python -m ruff check scripts/check_duplicate_invariants.py` 0 错。
- **边界声明**：`admin/` 包内未被 `__init__` 导入的孤儿模块（`broken_asset.py`/`found_asset.py`/`lost_asset.py`）非 G-5 射程（包内死文件，非包─模块并存），未处置，如需清理另立条目。

### A-31. 【关闭 2026-09-22】生命周期销毁删除写路径部门作用域收口（批次①：#10 限定域实施——A-29 的 lifecycle 面，B12 模式）
- **编号冲突规避**：顺延登记（A-30 已占用）。
- **状态**：✅ 已关闭（本次已修复） | 关闭日期：2026-09-22 | 本批修复
- **判定**：broken/lost/found/repair 四条删除链路（单删+批删）零 user 零部门作用域（Service 层 `delete_*_asset` 仅 `is_deleted=False` 直查），与 A-29（damaged 写路径）及 core `batch_delete_execute` 框架先例（B12 模式）不一致——系 v2.9.35 观察项③「批量删除框架时代预存量零部门作用域」在生命周期域对应面。
- **修复内容**：四单删方法（`delete_repair/broken/lost/found_asset`）加 `user: Any | None = None` 尾参，`get()` 后 `if user is not None and obj.asset_recordcode is not None: AssetSelector.ensure_asset_visible(obj.asset_recordcode, user)`（与 `_run_lifecycle_transition` 同口径，越权抛 `ASSET_NOT_VISIBLE`）；两批量方法（`batch_delete_lifecycle_asset`/`batch_delete_repair_asset`）加 user 并闭包逐条透传 → `ASSET_NOT_VISIBLE` 入 fail_items，**不经 ids 预筛（Option B 定案，success+fail==total 契约保持）**；四单删补 `# [HALT]`（§1.3 敏感操作标注）；View 三处透传 `user=request.user`（`_lifecycle_base.batch_delete` / `repair_asset_view.destroy` / `repair_asset_view.batch_delete`）。
...
- **批次③收口（v2.9.42）**：三 ViewSet 的手写 `batch_delete` action 本体随全仓 13 端点一并收敛至 `BatchDeleteViewMixin`（见 A-32），本 A-31 条目（Service 层 user 收口）与 A-32（View 层骨架收敛）形成闭环；越权语义保持：`batch_delete_passes_user=True` 逐条透传 user，`ASSET_NOT_VISIBLE` 入 fail_items。

### A-32. 【关闭 2026-09-22】批量删除视图层手写 action ×13 → BatchDeleteViewMixin（批次③：#10 三批修复计划最后一笔，A-21/A-15/A-31 的 view 面，B12 模式）
- **编号**：顺延登记（A-31 已占用）。**状态**：✅ 已关闭（本次已修复） | 关闭日期：2026-09-22 | 本批修复
- **判定**：克隆。13 个域的 `@action(detail=False, url_path="batch-delete")` 手写骨架逐字同构——`serializer 校验 → resolve_operator → Service 编排 → BatchResponseHelper.delete_response(message="批量删除完成,成功 X 条,失败 Y 条")`（A-21 已收 asset_service 单条守卫与批量执行框架、A-15 已收序列化器校验、A-31 已收生命周期 user 作用域，View 层仍存 13 份副本；证据 `rg "def batch_delete\(self" apps` = 13）。差异谱系：employee/department 不传 operator（Service 签名仅 `(ids)`）；damaged/repair/lifecycle/asset 需传 `user=request.user`（A-29/A-31 部门作用域 RBAC）；asset 需 ids 预筛（`get_queryset` 部门预筛）；lifecycle 需注入 `delete_service_method` 字符串（三子类唯一差异）。
- **修复内容**：核心 `core/batch_mixins.py::BatchDeleteViewMixin`（约 +50 行）：唯一 `batch_delete` action + message 模板契约沿用 `BatchResponseHelper.delete_response`；4 差异类属性 `batch_delete_serializer / batch_delete_service / batch_delete_passes_operator(默认 True) / batch_delete_passes_user(默认 False)`；2 差异钩子 `batch_delete_prefilter(默认恒等) / batch_delete_invoke(默认直调)`；`batch_delete_invoke` 对存于类属性的普通函数做 `__func__` 解绑——实例访问会被绑定到 ViewSet(self) 造成 Service 多收一个 self（mypy strict 下 `cast(dict[str, Any], ...)` 收口 warn_return_any）。13 处手写 action 全部删除改类属性声明：`damaged/repair/lifecycle base/asset` +passes_user；`department/employee` +passes_operator=False；`asset` 覆写 prefilter；`lifecycle base` 覆写 invoke 注入 `delete_service_method`；`repair` 专属 `@extend_schema` 文档移入类级 `@extend_schema_view(batch_delete=...)` 逐字保留。unregistered 独苗一并纳入收编：新建 `UnregisteredAssetBatchDeleteSerializer(BaseBatchDeleteSerializer)`（ids help_text="未登记资产编码列表"），视图挂 Mixin，弃手写 `if not ids` 400；全仓唯一非 `BaseBatchDeleteSerializer` 批删序列化器清零。新建 `core/tests/test_batch_delete_view_mixin.py` 4 用例（空 ids 200 noop / prefilter 透传改写 / 默认只传 operator 不传 user / passes_user+invoke 注入 user）。
- **行为等价/唯一漂移**：12+1 端点零改动回归（`test_b5_baseline_snapshot` / `test_batch_contract_snapshot` 全文锁定 message 模板与响应键集）；唯一行为漂移为 **unregistered 空 ids**：`400 "请提供要删除的 ID 列表"` → `200 {total:0, success_count:0, fail_count:0}`（与其余 12 端点对齐；前端双层禁用态防护已核实——表格按钮 `:disabled="selectedRows?.length === 0"`（如 ContractDetails.vue）与 `useDepartmentEmployeeActions.ts` 显式长度检查，空列表请求不可发出，风险边界已记录）。`status`/`error_response`/`BatchResponseHelper` 等失效 import 经 ruff 清理（净删 7 处）；`MAX_BATCH_SIZE` 无删除序列化器调用点（②b 已删），批量创建面不动。
- **Schema 漂移（M-3 非破坏性，已重导出）**：schema 基线实含 14 个 batch-delete 端点 description（自动取自旧 action docstring，生命周期版为过时的"支持 pk/recordcode 双模式查找…"、unregistered 版为已删除手写循环的实现细节）。收敛后统一为"批量删除(全仓统一骨架, 差异仅 serializer/service/operator·user 开关)"——旧文案描述的是已被删除的实现，保留即失真文档。已重导出 `api-schema-baseline.json` 随本批提交（description 类漂移，CI 仅警告不失败；repair 专属描述除外，见上）。
- **验证命令**：视图层守卫 `rg "def batch_delete\(self" apps --glob "*.py"` = 0（core 内 = 1）；`pytest apps/assetmanagement/tests apps/usermanagement/tests apps/unregisteredasset/tests -k batch` 140 passed；全量 `pytest apps -q` = 1053 passed（CT-5 绿，CT-1/CT-3 覆盖未退化）；ruff scoped（apps/core/utils）= 仅存量 1 处（HEAD 同款）+ 新增 0；mypy scoped 22 存量（=HEAD 基线，新引入 1 处已 cast 收口)；`manage.py check` no issues；护栏脚本 G-1~G-5 PASS；`spectacular --validate` 重导出随批提交。

### A-33. 【关闭 2026-09-23】update_outasset 使用地点手工同步 vs `_build_asset_people_update` 三行重复（DR-1，审查/拆分核查发现）
- **编号**：顺延登记（A-32 已占用）。**状态**：✅ 已关闭（本次已修复） | 关闭日期：2026-09-23 | 本批修复（BR-4 拆分连带收敛）
- **判定**：克隆。`out_asset_service.create_outasset` 路径已收敛至公共 helper `_build_asset_people_update(asset, applicant, manager, using_location)`（`out_asset_service.py:135-147`），同一文件 `update_outasset` 却传 `using_location=None` 后在函数体内手工重写同一 `if using_location is not None: asset.asset_using_location = ...; update_fields.append("asset_using_location")` 三段逻辑（原 :214-217）——两条写路径同步已落库的主表字段各自实现，违反 DR-1「业务逻辑唯一实现」。
- **修复内容**：`update_outasset` 改传 `update_data.get("outasset_using_location")` 至 `_build_asset_people_update`，删除函数体内手工三段重复（净删 3 行）。拆分后 `update_outasset` 53 → 37 逻辑行（BR-4 达标，该函数原为护栏「未登记超长函数」红灯唯一来源，本次行数回落自动清零）；同时抽出 `_build_update_audit_snapshot`（before/after 审计快照构建，26 行，原冗余内联）。
- **证据**：`rg "_build_asset_people_update"` 全仓仅 `out_asset_service.py` 3 处（def :135 + create :161 + update :227），using_location 落库同步单一实现；`check_function_length_guard.py` 由 FAIL（update_outasset 53>50）转 PASS（0/0）。
- **验证命令**：`python scripts/check_function_length_guard.py`（PASS exit=0）；`pytest apps/assetmanagement/tests/test_out_asset_service.py apps/assetmanagement/tests/test_out_asset_view_api.py -q`（含 `test_update_out_asset_using_location` 双表同步断言:视图层 + before/after 审计快照回归）；Service 覆盖率 ≥90% 专项目标。`python -m ruff check apps/assetmanagement/services/out_asset_service.py` + mypy scoped 0 新增。

### A-34. 【关闭 2026-09-23】core/constants.py 9 组枚举常量双源残留 → Model TextChoices 单一来源（DR-1），SECRET_KEY 硬编码兜底移除（B1）
- **编号**：顺延登记（A-33 已占用）。**状态**：✅ 已关闭 | 关闭日期：2026-09-23 | 本批修复
- **判定（问题 1 — DR-1）**：`core/constants.py` 残存 9 组 `*_CHOICES` 枚举列表，与各模型侧 TextChoices 同值同义（数据双源）。核查订正用户分析的两处失真：① `asset.py:218`/`storage.py:36` 引用的 `ASSET_STATUS_CHOICES`/`STORAGE_TYPE_CHOICES` 是**模型类自身属性**（`= AssetStatus.choices`/`= StorageType.choices`），非 core.constants 模块级列表，STATUS 清单早已收敛不存在"与现状矛盾"；② core.constants **唯一真实消费者**仅 2 处视图 label map（`damaged_asset_view.py:32/:211` APPROVAL、`storage_view.py:23/:84` STORAGE），其余 7 组为零消费死代码。另核查发现 `usermanagement/models.py:170` 员工状态仍为普通元组列表（非 TextChoices，第三处语义重复源）。
- **修复内容（Choices 收敛，零 DB 变化）**：
  1. 2 处视图 label map 改引模型 TextChoices（`dict(DamagedAsset.ApprovalStatus.choices)` / `dict(Storage.StorageType.choices)`，值 pending·approved·rejected / newasset·recycle·broken·damaged 完全同值），删除对应 core.constants import。
  2. 删除 `core/constants.py:13-87` 全部 9 组 `*_CHOICES`（ASSET_APPEARANCE/OUTASSET_TYPE/EMPLOYEE_STATUS/DEPARTMENT_STATUS/APPROVAL_STATUS/CONTRACT_TYPE/CONTRACT_SETTLEMENT/STORAGE_TYPE/HARDDISK_STATUS）；文件头注明定位（仅存分页/批量尺寸常量，枚举一律 Model TextChoices 禁止回流）。
  3. `EmployeeStatus(models.TextChoices)`（active/left/retirement 同值，usermanagement/models.py）替换普通元组列表，保留兼容别名 `EMPLOYEE_STATUS_CHOICES = EmployeeStatus.choices`（`base.py:371` ENUM_NAME_OVERRIDES 路径字符串与 selectors/views/service 引用 `Employee.EMPLOYEE_STATUS_CHOICES` 均不变）。
  4. **护栏**：`test_status_choices_singleton.py` 新增源码级断言 `"_CHOICES" not in core/constants.py`（防任何枚举回流，最简且不逐项枚举）。
- **修复内容（问题 2 — SECRET_KEY B1，SC-1 残余风险）**：`development.py` 硬编码兜底 `os.environ.get("SECRET_KEY", "*4h%...42")`（随仓库分发的真实密钥）改为 `config("SECRET_KEY", default=get_random_secret_key())`（decouple 读 .env/env 优先、运行时随机天生兜底，仓库零硬编码）；denylist 校验保留。配套：CI `backend-test` job 注入 `SECRET_KEY`（development settings 现为首选，避免踩新兜底路径不确定性）。关键核查订正：**pytest.ini 强制 `config.settings.development`**（非 test.py），`os.environ.get` 不读 .env（无 load_dotenv），故原方案"改 os.environ.get + 空值 fail-fast"会红全量测试。
- **行为等价**：枚举值/标签逐字节同值，schema `api-schema-baseline.json` diff **零漂移**（生成对比 `a == b` True）；`makemigrations --dry-run` `No changes detected`（choice 不改列类型，无 DB 变更）。
- **验证命令**：`python manage.py makemigrations --dry-run`（No changes）+ `python manage.py check`（no issues）；`python -m ruff check` scoped 6 文件（All checks passed）；`python -m pytest apps/assetmanagement apps/usermanagement -q`（858 passed：assetmanagement 750 + usermanagement 108）；`python scripts/check_duplicate_invariants.py`（PASS）；mypy scoped 与基线 20 errors in 8 files 完全一致（零新增）；`spectacular` 重导出 + `a == b` 比对 True。
- **回滚风险**：若未来向 core/constants.py 新增任何枚举常量，将触发 `_CHOICES` 护栏红灯（有意为之）。

### A-35. 【关闭 2026-09-23】common-forms.scss form-container 内嵌 .card-header 与独立 card-header mixin 同体样式双写（DR-2）
- **编号**：顺延登记。**状态**：✅ 已关闭 | 关闭日期：2026-09-23 | 本批修复
- **判定（DR-2）**：`common-forms.scss` 中 `form-container` mixin 内嵌 `.card-header` 块（原 :35-58）与文件末尾独立 `@mixin card-header`（原 :647-670，现 :626）**逐属性同值双写**：`display:flex / align-items:center / gap:12px / font-weight:600 / color:var(--color-primary) / padding:18px 24px / linear-gradient(135deg,#f0f7ff→#e6f0ff) / border-bottom / .el-icon 20px / span 16px`。行号相对先前记录（57-80/664-692）漂移 ~22 行，系 #44（commit 330c7d5）删除头部 15 个 SCSS 变量块所致；#44 仅做变量双轨收敛未触碰本结构。第三处 `info-card` 内嵌 `.card-header`（:241，简化版：无渐变底、text-dark、padding 12px 16px、`.section-icon`）视觉规格不同，**判定保留不合并**。
- **修复内容**：内嵌重复体（24 行）替换为 `.card-header { @include card-header; }`（3 行，净 -21 行）。**关键语义**：直接平铺 `@include card-header;` 会令 mixin 内的 `.el-icon`/`span` 子选择器编译为 form-container 内所有 `.el-icon`/`span` 匹配（选择器作用域污染），故必须保留外层 `.card-header {…}` 包裹，与消费端现状（`.section-header { @include card-header(); }`）同构。差异化核对免做：两处值已实测零差异，无需要保留的差异属性。
- **行为等价**：编译产物验证——`DamagedAssetForm`/`UserBatchImport` 均输出 `.box-card .card-header [data-v-*]` 含 `gap:12px`/渐变底/`.el-icon 20px`/`span 16px`，逐值一致；`DamagedAssetBasicDetails` 等详情页 `.section-header` 值不变。文件 670→624 行。
...
### B-21. 【已关闭 2026-09-22】写路径 IntegrityError→业务码兜底多实现（hard_disk_sn ×3 + damaged ×1 → core 单实现）
- **判定**：重复模式（DR-1 风险面）。同一语义「DB 唯一约束冲突 = 并发窗口穿透预检 → 映射为业务错误码而非通用 400」独立内联。**台账低估订正**：初记「双实现」，实测 `hard_disk_sn_service.py` 内部自身三份同构拷贝（:49 `create` / :99 `update` / :154 `batch_save`，均 `except IntegrityError` + 列 token `"harddisk_sn_code"` + `AppValidationError(DUPLICATE_SN_CODE) from exc` + 未命中裸 `raise`），叠加 `damaged_asset_service.py:63`（`create_damaged_asset`，PR-1 修复时按该先例新增，列 token=`asset_recordcode`、错误码 `DUPLICATE_DAMAGED_RECORD`）共 **4 调用点**。
- **来源**：审查报告 #15（B5 TOCTOU）修复过程中发现并登记（§1.8 新发现义务）。
...
| G-5 | 影子死文件禁止复现 | 包（含 `__init__.py`）与同级同名 `.py` 不得并存（包优先解析使 `.py` 成死代码），`apps/` 全递归扫描 |
...
- **v2.9.42 (2026-09-22)**：批次③ 批量删除视图层手写 action ×13 → `BatchDeleteViewMixin`（关闭 A-32，#10 三批修复计划最后一笔；A-21/A-15/A-31 的 view 面）——`core/batch_mixins.py` 新增唯一 `batch_delete` action 骨架（4 差异类属性 serializer/service/passes_operator/passes_user + 2 差异钩子 prefilter/invoke），`batch_delete_invoke` 对类属性普通函数 `__func__` 解绑（防实例绑定 ViewSet 致使 Service 多收 self）；12 框架 ViewSet + unregistered 13 处手写 action 全部删除改类属性声明（damaged/repair/lifecycle/asset +passes_user；department/employee +passes_operator=False；asset 覆写 prefilter；lifecycle 覆写 invoke 注入 delete_service_method；repair 专属 @extend_schema 移入 extend_schema_view 逐字保留）；unregistered 收编（新 `UnregisteredAssetBatchDeleteSerializer(BaseBatchDeleteSerializer)`，弃手写 `if not ids` 400 → 200 noop 对齐全仓）；新增 core Mixin 4 用例。唯一行为漂移=unregistered 空 ids 400→200 0/0（前端双层禁用态防护已核实）；schema 14 处 batch-delete description 由旧 docstring 统一为骨架文案（旧文案描述已删除实现，失真；M-3 非破坏性漂移，已重导出随 PR 提交）。验证：`rg "def batch_delete\(self" apps` = 0（core=1）、定向 batch 140 passed、全量 1053 passed、ruff scoped 0 新增、mypy scoped 22 = HEAD 存量 0 新增、`manage.py check` no issues、护栏 G-1~G-5 PASS、schema 重导出已提交。
- **v2.9.40 (2026-09-22)**：批次②a broken/lost/found destroy 硬删→软删对齐批量语义并承接批次①的 user 透传（A-31 边界声明闭环）——`_lifecycle_base.py::AssetLifecycleViewSetBase` 新增 `destroy` 覆写（镜像 repair_asset_view.destroy:136-145：`get_object()`（作用域 404 兜底）→ `getattr(AssetLifecycleMixin, delete_service_method)(..., user=request.user)` → `success_response(data={"recordcode": obj.recordcode}, message="删除成功")`），替换 `core/mixins.py::ResponseWrapperMixin.destroy`（:193-215，此前为 `perform_destroy` → `instance.delete()` 物理删除）；三视图集 destroy 由此与批量删除同语义（软删 + 审计 + `[HALT]` 已在 Service 层）。响应体 `data` 由 `{}` 变 `{recordcode}`——前端 `brokenAsset.ts:57` 仅 `unwrapResponse(request.delete<void>(...))` 消费 `code==0`，跨端契约未破坏。测试三处 destroy 断言改软删语义（`all_objects.filter(is_deleted=True)` + data.recordcode + 审计 DELETE 落库），repair 两用例不动。验证：view+snapshot 定向 48 passed + ruff 0 + mypy scoped 0 + `manage.py check` no issues。契约零变化，`api-schema-baseline.json` 无需重导出。
- **v2.9.39 (2026-09-22)**：批次① 生命周期销毁删除写路径部门作用域收口（关闭 A-31，B12 模式，#10 三批修复计划第一笔）——四单删方法（`delete_repair/broken/lost/found_asset`，asset_lifecycle_mixin.py:193/:227/:250/:290）加 `user: Any | None = None` 尾参并插行级校验（`if user is not None and obj.asset_recordcode is not None: AssetSelector.ensure_asset_visible(...)`，越权抛 `ASSET_NOT_VISIBLE`）；两批量方法（:344/:368）加 user 闭包逐条透传（Option B，不经 ids 预筛，success+fail==total 契约保持）；四单删补 `# [HALT]`；View 三处透传 `user=request.user`（_lifecycle_base.batch_delete / repair_asset_view.destroy/batch_delete）。新增 `TestDeleteServiceUserScope` 5 用例（部门 A/B 夹具镜像 A-29；软删断言经 `all_objects`——默认 manager 隐过滤软删行）。单独删/批量跨部门拒绝语义齐全，越权误删零发生。修复 `is_deleted` 断言初版走默认 manager 落空。验证：定向 52 passed + 生命周期 view 41 passed + b5 快照 8 passed + selector/rbac 32 passed + `manage.py check` no issues + ruff 0 + mypy-scoped 0 新增（37 存量均在他文件或测试既有 no-untyped-def 风格）+ 护栏 PASS。契约零变化，`api-schema-baseline.json` 无需重导出。
- **v2.9.38 (2026-09-22)**：关闭 B-21（DR-1 IntegrityError 兜底收敛）——`core/exceptions.py` 新增 `re_raise_or_map_integrity_error(exc, column_token, error_code, detail) -> NoReturn`（命中 token → `AppValidationError from exc`，未命中裸 `raise` 原样重抛）；4 调用点收敛（hard_disk_sn ×3：create/update/batch_save + damaged ×1：create_damaged_asset，各删 6-8 行内联改 1-4 行委托）。台账低估订正：hard_disk 内部实为 3 份同构拷贝非 1 份。落点零新增依赖（AppValidationError 同文件，两 service 已 import）；不复用 `exception_handler._parse_integrity_error`（handler 私有/泛化关键字/返回文案，与业务层职责不同层）。helper 裸 raise 重抛经 `python -c` 实证。门禁口径修正：`except IntegrityError` 子句按 Python 语法保留调用点（4 处），收敛的是 token 映射逻辑（全仓 `in str(exc)` 仅 core/exceptions.py:141、apps 服务层 token 映射归零）。验证：三套件 69 passed（语义锁）、ruff 3 改动文件 0 错、mypy scoped 3 文件 0 错（5 存量错误均在他文件：models/selectors）、`manage.py check` no issues。契约零变化，`api-schema-baseline.json` 无需重导出。
- **v2.9.37 (2026-09-22)**：G-5 通用护栏落地 + A-30 影子死文件治理（§1.8 新发现义务登记，A-30 条目见 A 区）——存量扫描实证两处 B-11/B-14 同型残留：`apps/assetmanagement/admin.py`（201 行、admin/ 包遮蔽、`asset_admin.py` 注册 16 模型为超集，整文件死代码）与 `apps/authusermanagement/tests.py`（1 行占位、tests/ 包遮蔽）；删除前先跑 admin 注册冒烟基数（registry 21 模型），删除后复跑逐字节一致 + `manage.py check` no issues，再整文件删除两处含 pyc；护栏新增 `check_g5_no_shadow_modules()`（rglob `__init__.py` 递归断言同名 `.py` 不得并存，覆盖 B-11 全部 5 路径 + B-14 + 深层嵌套），A-23 附带建议的窄化 services.py 检查由此通用检查取代；docstring 退出码注明 G-1~G-3、G-5 阻断。验证：先红后绿（touch services.py → 1 BLOCK → 补 admin.py → 2 BLOCK → 双删 → PASS exit=0）、`pytest apps/authusermanagement apps/assetmanagement -q` 824 passed、`python -m ruff check scripts/check_duplicate_invariants.py` 0 错、全仓护栏 PASS。契约零变化，`api-schema-baseline.json` 无需重导出。
