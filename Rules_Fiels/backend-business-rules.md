...
- v1.7 (2026-07-21): D-015 错误码体系重构——`response_utils.py` 新增 `BizCode` 类（~50 个四位业务码，与 HTTP 状态码完全解耦）+ `ERROR_CODE_TO_BIZ` 映射表（覆盖代码中 142 处 `error_code` 字符串）+ `EXCEPTION_TO_ERROR_TYPE` 语义化映射（不暴露 Python 类名）；`exception_handler.py` 重构为三级兜底（`exc.error_code` → 异常类型默认值 → HTTP 兜底），异常响应 `code` 字段从 HTTP 状态码改为业务码，`data` 字段新增 `error_type` 语义化标识。View 层 193 处 `error_response` 调用暂不修改（generic 错误默认 `2099`）。
